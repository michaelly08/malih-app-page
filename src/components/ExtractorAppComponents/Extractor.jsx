...
const Extractor = ({extractType}) => {
    ...
    useEffect(() => {
        ...
        setInactiveEmails([])
        setErrorEmails([])
    }, [guessByCheck, inputText, extractType])
            ...
            <div className="emailguesser-wrapper">
                <div className="emailguesser-box">
                    <form id="form" onSubmit={extractEmails}>
                        <label>What would you like to extract? <span style={{color: "red"}}>*</span></label>
                        <div className="jobroles-container" style={{height: "110px"}}>
                            {guessByOptions?.map((option, i) => (
                                <div key={option} className="email-checkboxes" style={{color: "white", fontSize: "17px", height: "100%"}}>
                                    <input type="checkbox" id={option} name={option} value={option} onChange={handleExtractCheck} checked={extractChecked(option)}></input>
                        ...
                        <label className="company-urls-label">
                            <div>
                                {extractType === "Text" ? "Enter Text To Extract" : "Upload CSV File(s)"} <span style={{color: "red"}}>*</span>
                            </div>
                            {extractType === "Text" ? 
                        ...
                        {extractType === "Text" ?
                            <textarea className="extractor-textarea" onChange={(e) => setInputText(e.target.value)} value={inputText}></textarea>
                            :
                        <>
                ...
                <div className="emailguesser-list-box">
                    ...
                    <div className="emailguesser-list">
                        ...
                        : <div className="emailguesser-noemails">
                            {guessByCheck?.length > 0 ? `${extractType === "Text" ? "Enter text" : "Upload csv file(s)"} and click \"Extract\" to get ${guessByCheck?.join(" & ").toLowerCase()}` : "Please select what you would like to extract"}
                            </div>}
                    </div>
